...
def bn_act(linout, perform_gcn, perform_bn, activation_fn, is_training,
    ...
    if perform_gcn:
        ...
        mean, variance = tf.compat.v1.nn.moments(linout, axes=[2], keep_dims=True)
    ...
    if perform_bn:
        with tf.compat.v1.variable_scope("bn", reuse=tf.compat.v1.AUTO_REUSE):
            linout = tf.compat.v1.layers.batch_normalization(
                inputs=linout,
...
def get_W_b_conv1d(in_channel, out_channel, ksize, dtype=None):
    ...
    W = tf.compat.v1.get_variable(
        ...
        initializer=tf.compat.v1.truncated_normal_initializer(stddev=2.0 / fanin),
...
def conv1d_resnet_block(inputs, ksize, nchannel, activation_fn, is_training,
    ...
    # Pass branch
    with tf.compat.v1.variable_scope("pass-branch"):
    ...
    # Conv branch
    with tf.compat.v1.variable_scope("conv-branch"):
        cur_in = inputs
        # Do bottle neck if necessary (Linear)
        if midchannel != nchannel:
            with tf.compat.v1.variable_scope("preconv"):
                cur_in = conv1d_layer(
        ...
        # Do bottle neck if necessary (Linear)
        if midchannel != nchannel:
            with tf.compat.v1.variable_scope("postconv"):
                cur_in = conv1d_layer(
                    inputs=cur_in, ksize=1,
                    nchannel=nchannel,
                    activation_fn=None,
...
def linear(input_, outputSize, activation_fn = None, name = 'linear'):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        w = tf.compat.v1.get_variable('w_linear', [shape[1], outputSize], tf.float32, tf.compat.v1.truncated_normal_initializer(stddev=0.1))
        b = tf.compat.v1.get_variable('bias', [outputSize], initializer=tf.constant_initializer(0.0))
...
def conv2d(x, outputDim, patchSize, stride, activation_fn=tf.nn.relu, padding='VALID', name='conv2d'):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        w = tf.compat.v1.get_variable('w', kernelShape, tf.float32, initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
        conv = tf.nn.conv2d(x, w, s, padding)

        b = tf.compat.v1.get_variable('bias', [outputDim], initializer=tf.constant_initializer(0.0))
